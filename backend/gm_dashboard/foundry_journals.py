...
def render_scene_journal_html(scene: dict, mirrored_assets: list[dict]) -> str:
    lines = [f"<h1>{scene['title']}</h1>"]
    if scene.get("description"):
        lines.append(f"<p>{scene['description']}</p>")
    if scene.get("purpose"):
        lines.append(f"<p><strong>Purpose:</strong> {scene['purpose']}</p>")
    if scene.get("opening_image"):
        lines.append(f"<p><strong>Opening image:</strong> {scene['opening_image']}</p>")
    if scene.get("sensory_words"):
        lines.append(f"<p><strong>Sensory words:</strong> {scene['sensory_words']}</p>")
    if scene.get("cast"):
        lines.append(f"<p><strong>Cast:</strong> {', '.join(scene['cast'])}</p>")
    if scene.get("location"):
        lines.append(f"<p><strong>Location:</strong> {', '.join(scene['location'])}</p>")
    if mirrored_assets:
        lines.append("<h2>Images</h2>")
        for asset in mirrored_assets:
            lines.append(f'<img src="{asset["foundry_path"]}" alt="{asset.get("title", "")}">')
    return "\n".join(lines)
